    ...
    <section className="container mb-5 mt-n4 position-relative" style={{ zIndex: 10 }}>
      <div className="bg-white shadow rounded-3 p-4">
        ...
        <div className="position-relative d-flex align-items-center">
          {/* Nút mũi tên TRÁI */}
          <button
            className="btn btn-danger rounded-circle position-absolute start-0 z-2 shadow d-flex justify-content-center align-items-center p-0 brand-scroll-btn"
            style={{ width: '32px', height: '32px', transform: 'translateX(-50%)' }}
            onClick={() => scroll(-250)}>
            <i className="bi bi-chevron-left text-white fw-bold"></i>
          </button>

          {/* Vùng chứa danh sách thương hiệu (Cuộn ngang) */}
          <div className="d-flex w-100 overflow-auto hide-scrollbar gap-2 gap-lg-5 px-4" ref={scrollRef}>
            {loading ? (
              ...
              brands.map(brand => (
                <Link key={brand.id} to={`/products?brand=${brand.id}`} className="brand-item text-center text-decoration-none text-dark flex-shrink-0" style={{ width: '100px' }}>
                  <div className="brand-img-wrapper d-flex align-items-center justify-content-center mb-2 mt-2" style={{ height: '60px' }}>
          ...
          {/* Nút mũi tên PHẢI */}
          <button
            className="btn btn-danger rounded-circle position-absolute end-0 z-2 shadow d-flex justify-content-center align-items-center p-0 brand-scroll-btn"
            style={{ width: '32px', height: '32px', transform: 'translateX(50%)' }}
            onClick={() => scroll(250)}>
